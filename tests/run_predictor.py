...
from libemg.emg_predictor import EMGRegressor, OnlineEMGRegressor, EMGClassifier, OnlineEMGClassifier
import re, json, os
...
if __name__ == '__main__':
    ...
    _, sm = delsys_streamer(channel_list=[1,4,7,15]) # returns streamer and the shared memory object, need to give in the active channels number -1, so 2 is sensor 3
    ...
    if regression_selected:
        ...
        emg_model.fit(feature_dictionary=data_set)
        classifier = OnlineEMGClassifier(emg_model, WINDOW_SIZE, WINDOW_INCREMENT, odh, feature_list, ip= UDP_IP, port= UDP_PORT, std_out=True)
        classifier.run(block=False) # block set to false so it will run in a seperate process.
