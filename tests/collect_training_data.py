...
if __name__ == "__main__":
    ...
    streamer, sm = delsys_streamer(channel_list=[2,4,5,9]) # returns streamer and the shared memory object, need to give in the active channels number -1, so 2 is sensor 3
    # Create online data handler to listen for the data
    odh = OnlineDataHandler(sm)
    filename = 'data/regression/test-11-05/C_0_R_0_emg.csv'

    sample_thread = threading.Thread(target=sample_emg_data, args=(24, 10, odh, filename))
    sample_thread.start()

    while time.perf_counter_ns()/1e9 < (rise_duration+steady_state_duration):
        pass